...
} from '@/ai/flows/extract-recipe-data-from-image';
import {
  simplifyInstructions
} from '@/ai/flows/simplify-instructions';
import {
  Button
...
const RecipeItemCard = ({
  ...
  const treatmentDurationValue = useWatch({
    control,
    name: `items.${index}.treatmentDurationValue`
  });
  const treatmentDurationUnit = useWatch({
    control,
    name: `items.${index}.treatmentDurationUnit`
  });
  ...
  const supplySource = useWatch({
    control,
    name: 'supplySource'
  });
  const requiresFractionationValue = useWatch({
    control,
  ...
  return ( <
    ...
    name = {
      `items.${index}.principalActiveIngredient`
    }
    render = {
      ({
        field
    ...
    name = {
      `items.${index}.pharmaceuticalForm`
    }
    render = {
      ({
        field
        ...
        Select onValueChange = {
          (value) => {
            ...
            if (defaults && appSettings) {
              setValue(`items.${index}.concentrationUnit`, defaults.concentrationUnit, {
                shouldValidate: true
              });
              setValue(`items.${index}.dosageUnit`, defaults.dosageUnit, {
                shouldValidate: true
              });
              setValue(`items.${index}.totalQuantityUnit`, defaults.totalQuantityUnit, {
                shouldValidate: true
              });
            }
          }
        ...
        <
        SelectContent > {
          appSettings?.pharmaceuticalForms.map(unit => < SelectItem key = {
            unit
          }
          value = {
            unit.toLowerCase()
    ...
    name = {
      `items.${index}.concentrationValue`
    }
    render = {
      ({
        field
      ...
      render = {
          ({
            ...
            <
            SelectContent > {
              appSettings?.concentrationUnits.map(unit => < SelectItem key = {
                unit
              }
              value = {
                unit
      ...
      render = {
          ...
          render = {
              ({
                ...
                <
                SelectContent > {
                  appSettings?.dosageUnits.map(unit => < SelectItem key = {
                    unit
                  }
                  value = {
                    unit
          ...
          render = {
              ({
                ...
                FormControl > < Input placeholder = "Ej: 8, 12, 24" {
                  ...field
                }
                /></FormControl >
                <
          ...
          name = {
            `items.${index}.treatmentDurationValue`
          }
          render = {
              ({
                field
              ...
              name = {
                `items.${index}.treatmentDurationUnit`
              }
              render = {
                  ({
                    field
                    ...
                    <
                    SelectContent > {
                      appSettings?.treatmentDurationUnits.map(unit => < SelectItem key = {
                        unit
                      }
                      value = {
                        unit
              ...
              name = {
                `items.${index}.totalQuantityValue`
              }
              render = {
                  ({
                    field
                  ...
                  render = {
                      ({
                        ...
                        <
                        SelectContent > {
                          appSettings?.quantityToPrepareUnits.map(unit => < SelectItem key = {
                            unit
                          }
                          value = {
                            unit
                  ...
                  render = {
                      ({
                        ...
                        FormControl > < Textarea placeholder = "Instrucciones de uso para el paciente..." {
                          ...field
                        }
                        /></FormControl >
                        <
                  ...
                  name = {
                    `items.${index}.requiresFractionation`
                  }
                  render = {
                    ({
                      field
...

                  {
                    supplySource === 'Insumos de Skol' && requiresFractionationValue && ( <
                      ...
                      name = {
                        `items.${index}.sourceInventoryItemId`
                      }
                      render = {
                        ({
                          field
                          ...
                          Seleccione la "caja"
                          o producto base del cual se obtendrá este preparado. <
                          /FormDescription> <
                          FormMessage / >
                          <
                          /FormItem>
                  ...
                  size = "sm"
                  onClick = {
                    () => handleSimplifyInstructions(index)
                  }
                  disabled = {
                    isSimplifying === index
                  } > {
          ...
          export function RecipeForm({
                    ...
                    if (recipeData) {
                      if (copyFromId) {
                        ...
                        toast({
                          ...
                          description: 'Revisa los datos y guarda la nueva receta.'
                        });
                      } else { // Editing
            ...
            React.useEffect(() => {
              ...
              fields.forEach((_item, index) => {
                form.setValue(`items.${index}.requiresFractionation`, isSkolSource, {
                  shouldValidate: true
                });
              });
            ...
            const handleExtractWithAI = async () => {
              ...
              try {
                const result = await extractRecipeDataFromImage({
                ...
                if (result.items && result.items.length > 0) {
                  const filledItems = result.items.map(item => ({ ...defaultItem,
                    ...item
                  }));
                ...
                toast({
                  ...
                  description: 'Los campos del formulario han sido pre-rellenados.'
                });
              } catch (error) {
                console.error('AI extraction failed:', error);
                toast({
                  title: 'Error de IA',
                  description: 'No se pudo extraer la información de la imagen.',
                  variant: 'destructive'
                });
              } finally {
            ...
            const handleSimplifyInstructions = async (index: number) => {
              ...
              if (!instructions) {
                toast({
                  ...
                  description: 'No hay texto para simplificar.',
                  variant: 'default'
                });
                return;
              }
              ...
              try {
                ...
                toast({
                  title: 'Error de IA',
                  description: 'No se pudo simplificar el texto.',
                  variant: 'destructive'
                });
              } finally {
            ...
            const onSubmit = async (data: RecipeFormValues) => {
              try {
                ...
                toast({
                  title: isEditMode && !copyFromId ? 'Receta Actualizada' : 'Receta Creada',
                ...
                toast({
                  ...
                  description: 'No se pudo guardar la receta. Por favor, intente de nuevo.',
                  variant: 'destructive'
                });
              }
            };
            ...
            return ( <
              ...
              <
              div className = "flex flex-col items-center justify-center p-6 border-2 border-dashed rounded-lg cursor-pointer hover:border-primary h-full"
              onClick = {
              ...
              /Button> <
              p className = "text-xs text-muted-foreground mt-2 text-center" > Rellena el formulario automáticamente a partir de la imagen. < /p> <
              /div> <
              /CardContent> <
              /Card>
              ...
              Input disabled value = {
                isEditMode && !copyFromId ? recipeId : "Nuevo (se genera al guardar)"
              }
              /> <
              /FormControl> <
              /FormItem> <
              ...
              render = {
                ({
                  ...
                  className = {
                    cn("w-full justify-between text-left font-normal", !field.value && "text-slate-500")
                  } > {
              ...
              render = {
                ({
                  ...
                  onValueChange = {
                    (value) => field.onChange(value as 'existing' | 'new')
                  }
                  className = "w-full" >
                  <
                  ...
                  render = {
                    ({
                      field
                    }) => ( < FormItem > < FormLabel > Nombre Paciente < /FormLabel><FormControl><Input placeholder="Nombre Apellido" {...field} / > < /FormControl><FormMessage / > < /FormItem>)} /
                    >
                    <
                    FormField control = {
                      form.control
                  ...
                  render = {
                    ({
                      field
                    }) => ( < FormItem > < FormLabel > RUT Paciente < /FormLabel><FormControl><Input placeholder="12.345.678-9" {...field} / > < /FormControl><FormMessage / > < /FormItem>)} /
                    >
                    <
                    /div> <
                    /TabsContent> <
              ...
              render = {
                ...
                render = {
                  ({
                    ...
                    render = {
                      ({
                        field
                      }) => ( < FormItem > < FormLabel > Nombre Médico < /FormLabel><FormControl><Input placeholder="Nombre Apellido" {...field} / > < /FormControl><FormMessage / > < /FormItem>)} /
                      >
                      <
                      FormField control = {
                        form.control
                      }
                    name = "newDoctorLicense"
                    render = {
                      ({
                        field
                    ...
                    render = {
                      ({
                        field
                      }) => ( < FormItem > < FormLabel > RUT Médico(Opcional) < /FormLabel><FormControl><Input placeholder="12.345.678-K" {...field} / > < /FormControl><FormMessage / > < /FormItem>)} /
                      >
                      <
                      FormField control = {
                        form.control
                      }
                    name = "newDoctorSpecialty"
                    render = {
                      ({
                        field
                      }) => ( < FormItem > < FormLabel > Especialidad Médico < /FormLabel><FormControl><Input placeholder="Ej: Cardiología" {...field} / > < /FormControl><FormMessage / > < /FormItem>)} /
                      >
                      <
                      /div> <
                      /TabsContent> <
                ...
                render = {
                  ({
                    ...
                    /FormControl> <
                    SelectContent > {
                      externalPharmacies.map(p => < SelectItem key = {
                        p.id
                      }
                      value = {
                        p.id
                  ...
                  isControlled && ( <
                    ...
                    render = {
                      ({
                        ...
                        FormControl >
                        <
                        Input placeholder = "Ej: F123456 o Folio Cheque" {
                          ...field
                        }
                        /> <
                        /FormControl> <
                    ...
                    <
                    Button type = "button"
                    variant = "outline" > Seleccionar archivo < /Button> <
